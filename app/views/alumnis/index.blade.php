    ...
    <div class="col-sm-12">
      <section class="panel panel-default">
        ...
        <table class="table table-bordered m-b-none"  data-ride="datatables">
          ...
          <tbody>
            @foreach($alumnis as $em)
            <tr class="@if( isset($em->user->throttle) && $em->user->throttle->banned == 1) blocked @endif" data-student="{{$em->user->id}}">
                {{--<td>{{$student->id}}</td>--}}
                <td><a href="{{URL::route('alumni.profile', $em->user->id)}}">{{$em->user->first_name}} {{$em->user->last_name}}</a></td>
                <td>{{$em->user->email}}</td>
                <td>{{$em->user->phone_number}}</td>
                <td>{{$em->user->cell_phone}}</td>
                <td>{{$em->organisation}}</td>
                <td>{{$em->designation}}</td>
                <td>{{$em->employment_status == "on" ? "Unemployed" : "Employed"}}</td>
                <td>
